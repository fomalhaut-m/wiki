...
![JVM体系结构](./assets/1566531069481.png)

## Java8以后的JVM

![JVM结构](./assets/1566540641226.png)
...
![GC作用域](./assets/1566541294021.png)

## 垃圾回收的算法

### 1. 引用计数

![引用计数](./assets/1566541747955.png)
...
![复制算法](./assets/1566542089129.png)

Java堆从GC的角度可以细分为**新生代（Eden、Form Survivor和To Survivor）**和老年代。

1. eden、FromSurvivor和ToSurvivor，年龄+1
   
   首先，由Eden区满的时候会触发一次GC，把还活着的对象拷贝到FromSurvivor区，当Eden区再次触发的时候，会扫描Eden区和FromSurvivor区，对这两个区进行垃圾回收，经过这次回收还存活的对象，则直接复制到ToSurvivor区（如果对象的年龄已经到达了老年的标准，则赋值到老年代区），同时把这些对象的年龄+1；
...
![标记整理](./assets/1566543204075.png)

2. 压缩：
   
   再次扫描，并整理空间（类似碎片整理）

![标记整理](./assets/1566543213143.png)
